...
func SetupRoutes(app *fiber.App, h *handlers.Handlers, m *middleware.Middleware) {
	...
	api.Get("/webhooks/:webhookID", h.Webhooks.GetWebhook)
	api.Patch("/webhooks/:webhookID", h.Webhooks.UpdateWebhook)
	api.Delete("/webhooks/:webhookID", h.Webhooks.DeleteWebhook)
