...
func TestRateLimit(t *testing.T) {
	...
	// RateLimit is fully implemented with Redis/memory/hybrid adapters
	// Test that it works without a configured limiter
	app := fiber.New()
	t.Cleanup(func() { app.Shutdown() })
...
func TestLogger(t *testing.T) {
	...
	// Logger is now fully implemented with structured logging
	// Test that it works and logs requests properly
	app := fiber.New()
	t.Cleanup(func() { app.Shutdown() })
	app.Use(m.Logger())
	app.Get("/test", func(c *fiber.Ctx) error {
