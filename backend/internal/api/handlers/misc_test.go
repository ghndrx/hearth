...
// =============================================================================
// Tests using actual handler methods (not inline route reimplementations)
// =============================================================================

// setupVoiceHandlerRealTestApp creates a test Fiber app using actual VoiceHandler
func setupVoiceHandlerRealTestApp(tb testing.TB) *fiber.App {
	app := fiber.New()
	tb.Cleanup(func() { _ = app.Shutdown() })

	// Use actual handler with nil service (nil service is handled gracefully)
	handler := NewVoiceHandler(nil)

	app.Get("/voice/regions", handler.GetRegions)
	app.Get("/voice/channels/:channelId/states", handler.GetChannelVoiceStates)
	app.Get("/voice/servers/:id/states", handler.GetServerVoiceStates)

	return app
}

// setupGatewayHandlerRealTestApp creates a test Fiber app using actual GatewayHandler
func setupGatewayHandlerRealTestApp(tb testing.TB) *fiber.App {
	app := fiber.New()
	tb.Cleanup(func() { _ = app.Shutdown() })

	// Use actual handler with nil gateway (nil gateway is handled gracefully)
	handler := NewGatewayHandler(nil)

	app.Get("/gateway/health", handler.Health)
	app.Get("/gateway/ready", handler.ReadinessCheck)
	app.Get("/gateway/live", handler.LivenessCheck)
	app.Get("/gateway/stats", handler.GetStats)

	return app
}

// --- VoiceHandler Real Method Tests ---

// Test VoiceHandler.GetRegions calls the actual handler method
func TestVoiceHandler_GetRegions_RealHandler(t *testing.T) {
	app := setupVoiceHandlerRealTestApp(t)
	...
	assert.Equal(t, "US West", result[0]["name"])
	assert.Equal(t, true, result[0]["optimal"])
	assert.Equal(t, "sydney", result[5]["id"])
}

// Test VoiceHandler.GetChannelVoiceStates with nil service returns empty array
func TestVoiceHandler_GetChannelVoiceStates_NilService(t *testing.T) {
	app := setupVoiceHandlerRealTestApp(t)

	req := httptest.NewRequest("GET", "/voice/channels/11111111-1111-1111-1111-111111111111/states", nil)
	resp, err := app.Test(req, -1)
...
// Test VoiceHandler.GetChannelVoiceStates with invalid channel ID returns 400
func TestVoiceHandler_GetChannelVoiceStates_InvalidChannelID(t *testing.T) {
	app := setupVoiceHandlerRealTestApp(t)

	req := httptest.NewRequest("GET", "/voice/channels/not-a-uuid/states", nil)
	resp, err := app.Test(req, -1)
...
// Test VoiceHandler.GetServerVoiceStates with nil service returns empty array
func TestVoiceHandler_GetServerVoiceStates_NilService(t *testing.T) {
	app := setupVoiceHandlerRealTestApp(t)

	req := httptest.NewRequest("GET", "/voice/servers/22222222-2222-2222-2222-222222222222/states", nil)
	resp, err := app.Test(req, -1)
...
// Test VoiceHandler.GetServerVoiceStates with invalid server ID returns 400
func TestVoiceHandler_GetServerVoiceStates_InvalidServerID(t *testing.T) {
	app := setupVoiceHandlerRealTestApp(t)

	req := httptest.NewRequest("GET", "/voice/servers/not-a-uuid/states", nil)
	resp, err := app.Test(req, -1)
...
// --- GatewayHandler Real Method Tests ---

// Test GatewayHandler.LivenessCheck always returns 200 with alive: true
func TestGatewayHandler_LivenessCheck_RealHandler(t *testing.T) {
	app := setupGatewayHandlerRealTestApp(t)

	req := httptest.NewRequest("GET", "/gateway/live", nil)
	resp, err := app.Test(req, -1)
...
// Test GatewayHandler.Health with nil gateway returns healthy status
func TestGatewayHandler_Health_NilGateway(t *testing.T) {
	app := setupGatewayHandlerRealTestApp(t)
...
// Test GatewayHandler.ReadinessCheck with nil gateway returns ready: true
func TestGatewayHandler_ReadinessCheck_NilGateway(t *testing.T) {
	app := setupGatewayHandlerRealTestApp(t)

	req := httptest.NewRequest("GET", "/gateway/ready", nil)
	resp, err := app.Test(req, -1)
...
// Test GatewayHandler.GetStats with nil gateway panics (known limitation)
// GetStats does NOT handle nil gateway, so we skip this test.
// The nil-safe behavior is tested via inline mock in TestGatewayHandler_GetStats_Empty
func TestGatewayHandler_GetStats_NilGateway_Skip(t *testing.T) {
	t.Skip("GetStats does not handle nil gateway - known limitation")
}
