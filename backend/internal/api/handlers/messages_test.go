...
func setupMessageTestApp(t *testing.T, messageService *mockMessageService) *fiber.App {
	app := fiber.New()
	...
	app.Get("/channels/:channelID/pins", func(c *fiber.Ctx) error {
		...
		if err != nil {
			...
			if errors.Is(err, services.ErrNoPermission) {
				return c.Status(fiber.StatusForbidden).JSON(fiber.Map{"error": "No permission to access this channel"})
			}
			return c.Status(fiber.StatusInternalServerError).JSON(fiber.Map{"error": err.Error()})
		}
...
func TestSendMessage_Success(t *testing.T) {
	...
	app := setupMessageTestApp(t, mockService)
...
func TestGetPinnedMessages_Success(t *testing.T) {
	...
	pinnedMessages := []*models.Message{
		{ID: uuid.New(), ChannelID: channelID, Content: "Pinned message 1", Pinned: true},
		{ID: uuid.New(), ChannelID: channelID, Content: "Pinned message 2", Pinned: true},
	}
	...
	if len(result) != 2 {
		t.Errorf("Expected 2 pinned messages, got %d", len(result))
	}
}
