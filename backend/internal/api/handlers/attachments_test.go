...
func setupAttachmentTestApp(t *testing.T, attachmentSvc *services.AttachmentService) (*fiber.App, uuid.UUID) {
	app := fiber.New()
	userID := uuid.New()
...
func TestAttachmentHandler_Upload(t *testing.T) {
	svc := services.NewAttachmentService(nil, nil, nil)
	app, _ := setupAttachmentTestApp(t, svc)
...
func TestAttachmentHandler_Get(t *testing.T) {
	svc := services.NewAttachmentService(nil, nil, nil)
	app, userID := setupAttachmentTestApp(t, svc)
...
//lint:ignore U1000 test helper for response body reading
