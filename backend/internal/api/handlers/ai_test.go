...
// --- AI Handler HTTP Tests ---

func TestGetProviders_Success(t *testing.T) {
	repo := newMockAIRepo()
	// Add a provider to the mock repo
	providerID := addTestProvider(repo)
	app, handler := setupAITestApp(t, repo)

	app.Get("/ai/providers", handler.GetProviders)

	req := httptest.NewRequest("GET", "/ai/providers", nil)
	resp, err := app.Test(req)
	...
	require.True(t, ok, "Expected providers to be an array")
	assert.Len(t, providers, 1, "Expected 1 provider")
	assert.Equal(t, providerID.String(), providers[0].(map[string]interface{})["id"])
}
...
func TestGetProvider_Success(t *testing.T) {
	...
	app.Get("/ai/providers/:id", handler.GetProvider)

	req := httptest.NewRequest("GET", "/ai/providers/"+providerID.String(), nil)
	resp, err := app.Test(req)
	require.NoError(t, err)
	assert.Equal(t, fiber.StatusOK, resp.StatusCode)

	result := readJSONResponse(t, resp)
	assert.Equal(t, providerID.String(), result["id"])
}
...
func TestGetProvider_NotFound(t *testing.T) {
	...
	req := httptest.NewRequest("GET", "/ai/providers/"+uuid.New().String(), nil)
	resp, err := app.Test(req)
	require.NoError(t, err)
...
func TestGetProvider_InvalidID(t *testing.T) {
	...
	req := httptest.NewRequest("GET", "/ai/providers/not-a-uuid", nil)
	resp, err := app.Test(req)
	require.NoError(t, err)
...
func TestCreateProvider_Success(t *testing.T) {
	...
	app.Post("/ai/providers", handler.CreateProvider)
	...
	jsonBody, _ := json.Marshal(body)
	req := httptest.NewRequest("POST", "/ai/providers", bytes.NewReader(jsonBody))
	req.Header.Set("Content-Type", "application/json")
...
func TestCreateProvider_InvalidBody(t *testing.T) {
	repo := newMockAIRepo()
	app, handler := setupAITestApp(t, repo)

	app.Post("/ai/providers", handler.CreateProvider)

	req := httptest.NewRequest("POST", "/ai/providers", strings.NewReader("invalid json"))
	req.Header.Set("Content-Type", "application/json")
...
func TestUpdateProvider_Success(t *testing.T) {
	...
	app.Patch("/ai/providers/:id", handler.UpdateProvider)
	...
	jsonBody, _ := json.Marshal(body)
	req := httptest.NewRequest("PATCH", "/ai/providers/"+providerID.String(), bytes.NewReader(jsonBody))
	req.Header.Set("Content-Type", "application/json")
...
func TestUpdateProvider_NotFound(t *testing.T) {
	...
	jsonBody, _ := json.Marshal(body)
	req := httptest.NewRequest("PATCH", "/ai/providers/"+uuid.New().String(), bytes.NewReader(jsonBody))
	req.Header.Set("Content-Type", "application/json")
...
func TestDeleteProvider_Success(t *testing.T) {
	...
	app.Delete("/ai/providers/:id", handler.DeleteProvider)

	req := httptest.NewRequest("DELETE", "/ai/providers/"+providerID.String(), nil)
	resp, err := app.Test(req)
	require.NoError(t, err)
...
func TestDeleteProvider_NotFound(t *testing.T) {
	...
	// Note: The mock's DeleteProviderConfig always succeeds (returns nil),
	// so we test the "not found" case via the handler's error response path.
	// Since the mock doesn't enforce existence, this tests the handler's
	// error handling when the service returns no error for a non-existent provider.
	req := httptest.NewRequest("DELETE", "/ai/providers/"+uuid.New().String(), nil)
	resp, err := app.Test(req)
	require.NoError(t, err)
	// Mock doesn't enforce existence, so returns 204
	assert.Equal(t, fiber.StatusNoContent, resp.StatusCode)
}
...
func TestGetUserCredentials_Success(t *testing.T) {
	...
	// Add a credential to the mock
	key := userID.String() + "-" + providerID.String()
	repo.credentials[key] = &ai.UserAICredential{
		ID:         uuid.New(),
		UserID:     userID,
	...
	app, handler := setupAITestApp(t, repo)
	app.Get("/ai/credentials", handler.GetUserCredentials)

	req := httptest.NewRequest("GET", "/ai/credentials", nil)
	req.Header.Set("X-Test-User-ID", userID.String())
	resp, err := app.Test(req)
	require.NoError(t, err)
	assert.Equal(t, fiber.StatusOK, resp.StatusCode)
...
func TestGetUserCredentials_Unauthorized(t *testing.T) {
	repo := newMockAIRepo()
	app, handler := setupAITestApp(t, repo)
...
func TestSetUserCredentials_Success(t *testing.T) {
	repo := newMockAIRepo()
	userID := uuid.New()
	providerID := addTestProvider(repo)
	app, handler := setupAITestApp(t, repo)

	app.Post("/ai/credentials", handler.SetUserCredentials)
	...
	jsonBody, _ := json.Marshal(body)
	req := httptest.NewRequest("POST", "/ai/credentials", bytes.NewReader(jsonBody))
	req.Header.Set("Content-Type", "application/json")
	req.Header.Set("X-Test-User-ID", userID.String())
...
func TestSetUserCredentials_InvalidProviderID(t *testing.T) {
	repo := newMockAIRepo()
	userID := uuid.New()
	app, handler := setupAITestApp(t, repo)
...
func TestDeleteUserCredential_Success(t *testing.T) {
	repo := newMockAIRepo()
	userID := uuid.New()
	providerID := addTestProvider(repo)
	...
	app, handler := setupAITestApp(t, repo)
	app.Delete("/ai/credentials/:providerId", handler.DeleteUserCredential)

	req := httptest.NewRequest("DELETE", "/ai/credentials/"+providerID.String(), nil)
	req.Header.Set("X-Test-User-ID", userID.String())
	resp, err := app.Test(req)
	require.NoError(t, err)
	assert.Equal(t, fiber.StatusNoContent, resp.StatusCode)
...
func TestDeleteUserCredential_InvalidID(t *testing.T) {
	repo := newMockAIRepo()
	userID := uuid.New()
	app, handler := setupAITestApp(t, repo)
	...
	req := httptest.NewRequest("DELETE", "/ai/credentials/not-a-uuid", nil)
	req.Header.Set("X-Test-User-ID", userID.String())
	resp, err := app.Test(req)
	require.NoError(t, err)
	assert.Equal(t, fiber.StatusBadRequest, resp.StatusCode)
...
func TestDeleteUserCredential_NotFound(t *testing.T) {
	repo := newMockAIRepo()
	userID := uuid.New()
	providerID := uuid.New() // Not in repo
	app, handler := setupAITestApp(t, repo)
...
func TestGetModelRoutings_Success(t *testing.T) {
	repo := newMockAIRepo()
	app, handler := setupAITestApp(t, repo)

	app.Get("/ai/routing", handler.GetModelRoutings)

	req := httptest.NewRequest("GET", "/ai/routing", nil)
	resp, err := app.Test(req)
	require.NoError(t, err)
	assert.Equal(t, fiber.StatusOK, resp.StatusCode)
	...
	result := readJSONResponse(t, resp)
	_, ok := result["routing"]
	assert.True(t, ok, "Expected 'routing' key in response")
}

func TestSetModelRouting_Success(t *testing.T) {
	repo := newMockAIRepo()
	providerID := addTestProvider(repo)
	app, handler := setupAITestApp(t, repo)

	app.Post("/ai/routing", handler.SetModelRouting)
	...
	jsonBody, _ := json.Marshal(body)
	req := httptest.NewRequest("POST", "/ai/routing", bytes.NewReader(jsonBody))
	req.Header.Set("Content-Type", "application/json")
...
func TestSetModelRouting_InvalidProviderID(t *testing.T) {
	repo := newMockAIRepo()
	app, handler := setupAITestApp(t, repo)
...
func TestDeleteModelRouting_Success(t *testing.T) {
	repo := newMockAIRepo()
	routingID := uuid.New()
	repo.routings[routingID] = &ai.ModelRouting{
		ID:      routingID,
		Feature: ai.FeatureChat,
	}
	app, handler := setupAITestApp(t, repo)

	app.Delete("/ai/routing/:id", handler.DeleteModelRouting)

	req := httptest.NewRequest("DELETE", "/ai/routing/"+routingID.String(), nil)
	resp, err := app.Test(req)
	require.NoError(t, err)
	assert.Equal(t, fiber.StatusNoContent, resp.StatusCode)
}
...
func TestGetAvailableModels_Success(t *testing.T) {
	repo := newMockAIRepo()
	// Add an enabled provider so ListAvailableModels doesn't return nil
	providerID := addTestProvider(repo)
	repo.providers[providerID].IsEnabled = true
	app, handler := setupAITestApp(t, repo)

	app.Get("/ai/models", handler.GetAvailableModels)
...
func TestGetAvailableModels_WithUserID(t *testing.T) {
	repo := newMockAIRepo()
	userID := uuid.New()
	app, handler := setupAITestApp(t, repo)
...
func TestGetProviderTypes_Success(t *testing.T) {
	...
	app.Get("/ai/provider-types", handler.GetProviderTypes)

	req := httptest.NewRequest("GET", "/ai/provider-types", nil)
	resp, err := app.Test(req)
	require.NoError(t, err)
	assert.Equal(t, fiber.StatusOK, resp.StatusCode)

	result := readJSONResponse(t, resp)
	providerTypes, ok := result["provider_types"].([]interface{})
	require.True(t, ok)
	assert.NotEmpty(t, providerTypes)
}

func TestGetFeatureTypes_Success(t *testing.T) {
	repo := newMockAIRepo()
	app, handler := setupAITestApp(t, repo)

	app.Get("/ai/feature-types", handler.GetFeatureTypes)

	req := httptest.NewRequest("GET", "/ai/feature-types", nil)
	resp, err := app.Test(req)
	require.NoError(t, err)
	assert.Equal(t, fiber.StatusOK, resp.StatusCode)

	result := readJSONResponse(t, resp)
	featureTypes, ok := result["feature_types"].([]interface{})
	require.True(t, ok)
...
func TestHealthCheck_Success(t *testing.T) {
	...
	app.Get("/ai/health", handler.HealthCheck)

	req := httptest.NewRequest("GET", "/ai/health", nil)
	resp, err := app.Test(req)
	require.NoError(t, err)
	assert.Equal(t, fiber.StatusOK, resp.StatusCode)

	result := readJSONResponse(t, resp)
	_, hasProviders := result["providers"]
...
func TestUpdateUserSettings_Success(t *testing.T) {
	...
	app.Put("/ai/settings", handler.UpdateUserSettings)
	...
	jsonBody, _ := json.Marshal(body)
	req := httptest.NewRequest("PUT", "/ai/settings", bytes.NewReader(jsonBody))
	req.Header.Set("Content-Type", "application/json")
	req.Header.Set("X-Test-User-ID", userID.String())
	...
	result := readJSONResponse(t, resp)
	assert.Equal(t, "Settings updated successfully", result["message"])
}

func TestUpdateUserSettings_InvalidBody(t *testing.T) {
	repo := newMockAIRepo()
	userID := uuid.New()
	app, handler := setupAITestApp(t, repo)
	...
	req := httptest.NewRequest("PUT", "/ai/settings", strings.NewReader("not json"))
	req.Header.Set("Content-Type", "application/json")
	req.Header.Set("X-Test-User-ID", userID.String())
...
func TestSetAdminDefaults_Forbidden(t *testing.T) {
	repo := newMockAIRepo()
	app, handler := setupAITestApp(t, repo)

	app.Post("/ai/admin/defaults", handler.SetAdminDefaults)
	...
	jsonBody, _ := json.Marshal(body)
	req := httptest.NewRequest("POST", "/ai/admin/defaults", bytes.NewReader(jsonBody))
	req.Header.Set("Content-Type", "application/json")

	resp, err := app.Test(req)
	require.NoError(t, err)
	// Without admin flag in context, should be forbidden
	assert.Equal(t, fiber.StatusForbidden, resp.StatusCode)
}

func TestGetAdminDefaults_Forbidden(t *testing.T) {
	repo := newMockAIRepo()
	app, handler := setupAITestApp(t, repo)

	app.Get("/ai/admin/defaults", handler.GetAdminDefaults)

	req := httptest.NewRequest("GET", "/ai/admin/defaults", nil)
	resp, err := app.Test(req)
	require.NoError(t, err)
	// Without admin flag in context, should be forbidden
	assert.Equal(t, fiber.StatusForbidden, resp.StatusCode)
...
func TestGetUserSettings_Success(t *testing.T) {
	...
	app.Get("/ai/settings", handler.GetUserSettings)

	req := httptest.NewRequest("GET", "/ai/settings", nil)
	req.Header.Set("X-Test-User-ID", userID.String())
	resp, err := app.Test(req)
	require.NoError(t, err)
	assert.Equal(t, fiber.StatusOK, resp.StatusCode)
...
func TestGetUserSettings_Unauthorized(t *testing.T) {
	repo := newMockAIRepo()
	app, handler := setupAITestApp(t, repo)
