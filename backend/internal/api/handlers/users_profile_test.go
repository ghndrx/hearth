...
func (m *MockMutualFriendsService) GetCustomStatus(ctx context.Context, userID uuid.UUID) (*models.UserCustomStatus, error) {
	args := m.Called(ctx, userID)
	if args.Get(0) == nil {
		return nil, args.Error(1)
	}
...
func (m *MockMutualFriendsService) SetCustomStatus(ctx context.Context, userID uuid.UUID, req *models.UpdateStatusRequest) (*models.UserCustomStatus, error) {
...
func (m *MockMutualFriendsService) ClearCustomStatus(ctx context.Context, userID uuid.UUID) error {
	args := m.Called(ctx, userID)
	return args.Error(0)
}

// newProfileTestApp creates a Fiber app configured with the HTTPError handler.
func newProfileTestApp() *fiber.App {
...
// stringPtr is defined in ai_chat_test.go
