...
func (m *MockUserService) GetCustomStatus(ctx context.Context, userID uuid.UUID) (*models.UserCustomStatus, error) {
	args := m.Called(ctx, userID)
	if args.Get(0) == nil {
		return nil, args.Error(1)
	}
...
func (m *MockUserService) SetCustomStatus(ctx context.Context, userID uuid.UUID, req *models.UpdateStatusRequest) (*models.UserCustomStatus, error) {
...
func (m *MockUserService) ClearCustomStatus(ctx context.Context, userID uuid.UUID) error {
	args := m.Called(ctx, userID)
	return args.Error(0)
}
...
func newTestUserHandler(t *testing.T) *testUserHandler {
...
func TestUserHandler_GetMyDMs(t *testing.T) {
	...
	channels := []*models.Channel{
		{
			...
			Recipients: []uuid.UUID{th.userID, otherUserID},
			CreatedAt:  time.Now(),
		},
	}

	th.channelService.On("GetUserDMs", mock.Anything, th.userID).Return(channels, nil)
	th.userService.On("GetUser", mock.Anything, otherUserID).Return(&models.User{
		ID:            otherUserID,
		Username:      "testuser",
		Discriminator: "0001",
		CreatedAt:     time.Now(),
	...
	assert.Len(t, result[0].Recipients, 1)
	assert.Equal(t, "testuser", result[0].Recipients[0].Username)
...
func TestUserHandler_CreateDM(t *testing.T) {
	...
	th.userService.On("GetUser", mock.Anything, recipientID).Return(&models.User{
		ID:            recipientID,
		Username:      "recipient",
		Discriminator: "0001",
		CreatedAt:     time.Now(),
	...
	assert.Equal(t, "recipient", result.Recipients[0].Username)
...
func TestUserHandler_CreateDM_MissingRecipient(t *testing.T) {
	...
	json.NewDecoder(resp.Body).Decode(&result)
	assert.Equal(t, "recipient_id is required", result["message"])
}
...
func TestUserHandler_CreateDM_InvalidRecipient(t *testing.T) {
	...
	var result map[string]interface{}
	json.NewDecoder(resp.Body).Decode(&result)
	assert.Equal(t, "invalid recipient_id", result["message"])
}
...
func TestUserHandler_CreateDM_Self(t *testing.T) {
	...
	var result map[string]interface{}
	json.NewDecoder(resp.Body).Decode(&result)
	assert.Equal(t, "cannot create DM with yourself", result["message"])
}
...
func TestUserHandler_CreateGroupDM(t *testing.T) {
	...
	th.userService.On("GetUser", mock.Anything, th.userID).Return(&models.User{
		ID:            th.userID,
		Username:      "owner",
		Discriminator: "0001",
		CreatedAt:     time.Now(),
	}, nil)
	th.userService.On("GetUser", mock.Anything, recipient1).Return(&models.User{
		ID:            recipient1,
		Username:      "user1",
		Discriminator: "0001",
		CreatedAt:     time.Now(),
	}, nil)
	th.userService.On("GetUser", mock.Anything, recipient2).Return(&models.User{
		ID:            recipient2,
		Username:      "user2",
		Discriminator: "0001",
		CreatedAt:     time.Now(),
...
func TestUserHandler_CreateGroupDM_NoRecipients(t *testing.T) {
	...
	var result map[string]interface{}
	json.NewDecoder(resp.Body).Decode(&result)
	assert.Equal(t, "at least one recipient is required", result["message"])
}
...
func TestUserHandler_CreateGroupDM_TooManyRecipients(t *testing.T) {
	...
	var result map[string]interface{}
	json.NewDecoder(resp.Body).Decode(&result)
	assert.Equal(t, "group DM can have at most 10 members", result["message"])
}
...
func TestUserHandler_CreateGroupDM_InvalidRecipient(t *testing.T) {
	...
	var result map[string]interface{}
	json.NewDecoder(resp.Body).Decode(&result)
	assert.Contains(t, result["message"], "invalid recipient_id")
}
...
func TestUserHandler_CreateGroupDM_OnlySelf(t *testing.T) {
	...
	var result map[string]interface{}
	json.NewDecoder(resp.Body).Decode(&result)
	assert.Equal(t, "at least one other recipient is required", result["message"])
}
...
func newTestUserHandlerWithStorage(t *testing.T) *testUserHandlerWithStorage {
	th := newTestUserHandler(t)
...
func TestUserHandler_UpdateAvatar_NoFile(t *testing.T) {
	th := newTestUserHandlerWithStorage(t)
	...
	var result map[string]interface{}
	json.NewDecoder(resp.Body).Decode(&result)
	assert.Equal(t, "avatar file required", result["message"])
}
