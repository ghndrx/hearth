...
type Invite struct {
	...
	Temporary  bool       `json:"temporary" db:"temporary"`
	IsVanity   bool       `json:"is_vanity" db:"is_vanity"`
	VanityCode *string    `json:"vanity_code,omitempty" db:"vanity_code"`
	CreatedAt  time.Time  `json:"created_at" db:"created_at"`
...
// InviteUseLog tracks individual invite uses for analytics
type InviteUseLog struct {
	ID               uuid.UUID `json:"id" db:"id"`
...
// InviteAnalytics contains aggregated analytics for an invite
type InviteAnalytics struct {
	Code           string         `json:"code"`
	TotalUses      int            `json:"total_uses"`
	RecentUses     int            `json:"recent_uses_24h"`
	NewAccountJoin int            `json:"new_account_joins"`
	UseLogs        []InviteUseLog `json:"use_logs,omitempty"`
}
...
type CreateInviteRequest struct {
	...
	VanityCode *string `json:"vanity_code,omitempty"` // custom vanity code
}
