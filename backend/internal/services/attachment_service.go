...
func NewAttachmentService(storageService *storage.Service, quotaService *QuotaService, storageRepo StorageRepository) *AttachmentService {
...
func (s *AttachmentService) UploadWithAltText(
	...
	if s.quotaService != nil {
		if err := s.quotaService.CheckStorageQuota(ctx, uploaderID, nil, file.Size); err != nil {
			return nil, err
		}
	}
...
func (s *AttachmentService) Delete(ctx context.Context, attachmentID uuid.UUID, requesterID uuid.UUID) error {
	...
	// Update storage usage after successful delete
	if s.storageRepo != nil {
...
func (s *AttachmentService) DeleteByMessage(ctx context.Context, messageID uuid.UUID) error {
	...
	var deletedInfo = make(map[uuid.UUID]attachmentInfo)
	for id, a := range s.attachments {
		if a.MessageID == messageID {
			toDelete = append(toDelete, id)
			deletedInfo[id] = attachmentInfo{uploaderID: a.UploaderID, size: a.Size}
		}
	}
	...
	// Update storage usage for all affected uploaders
	if s.storageRepo != nil {
		// Group sizes by uploader
		uploaderSizes := make(map[uuid.UUID]int64)
		for _, info := range deletedInfo {
			uploaderSizes[info.uploaderID] += info.size
		}
...
//lint:ignore U1000 scaffold for future direct download support
