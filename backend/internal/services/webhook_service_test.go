...
func (m *MockChannelRepositoryForWebhook) GetPermissionOverrides(ctx context.Context, channelID uuid.UUID) ([]models.PermissionOverride, error) {
	args := m.Called(ctx, channelID)
	if args.Get(0) == nil {
		return nil, args.Error(1)
	}
...
func (m *MockChannelRepositoryForWebhook) RemoveRecipient(ctx context.Context, channelID, userID uuid.UUID) error {
	args := m.Called(ctx, channelID, userID)
	return args.Error(0)
}

func (m *MockChannelRepositoryForWebhook) CountRecipients(ctx context.Context, channelID uuid.UUID) (int, error) {
	args := m.Called(ctx, channelID)
	return args.Int(0), args.Error(1)
}

func (m *MockChannelRepositoryForWebhook) BulkUpdatePositions(ctx context.Context, entries []models.ReorderChannelEntry) error {
...
func (m *MockServerRepoForWebhook) GetMembersPaginated(ctx context.Context, serverID uuid.UUID, cursor *models.MemberCursor, limit int) (*models.PaginatedMembers, error) {
...
func (m *MockServerRepoForWebhook) GetMembersWithRole(ctx context.Context, serverID, roleID uuid.UUID) ([]*models.Member, error) {
...
func (m *MockServerRepoForWebhook) GetInviteByVanityCode(ctx context.Context, vanityCode string) (*models.Invite, error) {
	return nil, nil
}
func (m *MockServerRepoForWebhook) LogInviteUse(ctx context.Context, log *models.InviteUseLog) error {
	return nil
}
func (m *MockServerRepoForWebhook) GetInviteUseLogs(ctx context.Context, inviteCode string) ([]models.InviteUseLog, error) {
	return nil, nil
}
func (m *MockServerRepoForWebhook) GetServerInviteUseLogs(ctx context.Context, serverID uuid.UUID) ([]models.InviteUseLog, error) {
	return nil, nil
}

// MockMessageServiceForWebhook is a mock implementation of MessageService for webhook tests
type MockMessageServiceForWebhook struct {
	mock.Mock
}

func (m *MockMessageServiceForWebhook) SendMessage(ctx context.Context, authorID, channelID uuid.UUID, content string, attachments []*models.Attachment, replyTo *uuid.UUID) (*models.Message, error) {
	args := m.Called(ctx, authorID, channelID, content, attachments, replyTo)
	if args.Get(0) == nil {
		return nil, args.Error(1)
	}
...
func (m *MockMessageServiceForWebhook) SendMessageForWebhook(ctx context.Context, req SendWebhookMessageRequest) (*models.Message, error) {
...
func TestWebhookService_ExecuteWebhook_Success(t *testing.T) {
	...
	testMessage := &models.Message{
		...
		Author: &models.PublicUser{
			...
			Flags:    models.UserFlagSystemBot,
		},
		CreatedAt: time.Now(),
	}
	...
	assert.True(t, message.Author.Flags&models.UserFlagSystemBot != 0)
}
...
func TestExecuteWebhook_CreatesPersistedMessage(t *testing.T) {
	...
	// The message returned should have an ID, indicating it was persisted
	testMessage := &models.Message{
		ID:        messageID,
		ChannelID: channelID,
		AuthorID:  webhookID,
	...
	// Verify that SendMessageForWebhook is called with correct parameters
	msgSvc.On("SendMessageForWebhook", ctx, mock.MatchedBy(func(req SendWebhookMessageRequest) bool {
		return req.WebhookID == webhookID &&
	...
	assert.Equal(t, "This is a persisted message!", message.Content)
	assert.Equal(t, webhookID, message.AuthorID)
...
func TestExecuteWebhook_UsernameOverride(t *testing.T) {
	...
	channelID := uuid.New()
	webhookID := uuid.New()
	customUsername := "Custom Bot Name"
	...
	// Verify that the custom username is passed to SendMessageForWebhook
	msgSvc.On("SendMessageForWebhook", ctx, mock.MatchedBy(func(req SendWebhookMessageRequest) bool {
		return req.Username != nil && *req.Username == customUsername
	})).Return(testMessage, nil)
...
func TestExecuteWebhook_AvatarOverride(t *testing.T) {
	...
	// Verify that the custom avatar is passed to SendMessageForWebhook
	msgSvc.On("SendMessageForWebhook", ctx, mock.MatchedBy(func(req SendWebhookMessageRequest) bool {
	...
	assert.NoError(t, err)
	assert.NotNil(t, message)
	assert.NotNil(t, message.Author.AvatarURL)
	assert.Equal(t, customAvatar, *message.Author.AvatarURL)
}
