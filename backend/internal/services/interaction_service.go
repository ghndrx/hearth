...
// NewInteractionServiceWithEventBus creates a new interaction service with event bus
func NewInteractionServiceWithEventBus(
	repo InteractionRepository,
	slashCmdService *SlashCommandService,
	webhookService WebhookCommander,
	cache CacheService,
...
func (s *InteractionService) HandleInteraction(ctx context.Context, interaction *models.Interaction) (*models.InteractionResponse, error) {
	...
	// Publish interaction event if event bus is configured
	if s.eventBus != nil {
		s.publishInteractionEvent(interaction)
	}
...
func (s *InteractionService) publishInteractionEvent(interaction *models.Interaction) {
	eventData := &InteractionCreatedEvent{
		ID:        interaction.ID,
	...
	switch interaction.Type {
	case models.InteractionTypeApplicationCommand:
		s.eventBus.Publish(events.InteractionCreated, eventData)
	case models.InteractionTypeAutocomplete:
		s.eventBus.Publish(events.AutocompleteCreated, eventData)
	case models.InteractionTypeModalSubmit:
		s.eventBus.Publish(events.ModalSubmitted, eventData)
	default:
		s.eventBus.Publish(events.InteractionCreated, eventData)
	}
}

// InteractionCreatedEvent is published when an interaction is created
...
// CommandResponseSentEvent is published when a command response is sent
type CommandResponseSentEvent struct {
	Token     string
	AppID     uuid.UUID
	ChannelID uuid.UUID
	ServerID  *uuid.UUID
