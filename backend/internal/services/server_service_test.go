...
func (m *MockRoleRepository) GetMemberPermissions(ctx context.Context, serverID, userID uuid.UUID) (int64, error) {
	args := m.Called(ctx, serverID, userID)
	return args.Get(0).(int64), args.Error(1)
}

func (m *MockRoleRepository) GetDefaultRole(ctx context.Context, serverID uuid.UUID) (*models.Role, error) {
	args := m.Called(ctx, serverID)
	if args.Get(0) == nil {
		return nil, args.Error(1)
	}
...
func newTestServerService() (*ServerService, *MockServerRepository, *MockChannelRepository, *MockRoleRepository, *MockCacheService, *MockEventBus) {
	...
	quotaService := NewQuotaService(quotaConfig, serverRepo, quotaUserRepo, roleRepo, nil)

	service := NewServerService(serverRepo, channelRepo, roleRepo, nil, quotaService, nil, cache, eventBus)
...
func TestJoinServer_Success(t *testing.T) {
	...
	serverRepo.On("LogInviteUse", ctx, mock.AnythingOfType("*models.InviteUseLog")).Return(nil)
