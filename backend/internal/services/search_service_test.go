...
func (m *MockUserRepositoryForSearch) GetCustomStatus(ctx context.Context, userID uuid.UUID) (*models.UserCustomStatus, error) {
	args := m.Called(ctx, userID)
	if args.Get(0) == nil {
		return nil, args.Error(1)
	}
...
func (m *MockUserRepositoryForSearch) SetCustomStatus(ctx context.Context, status *models.UserCustomStatus) error {
	args := m.Called(ctx, status)
	return args.Error(0)
}

func (m *MockUserRepositoryForSearch) DeleteCustomStatus(ctx context.Context, userID uuid.UUID) error {
	args := m.Called(ctx, userID)
	return args.Error(0)
}
...
func TestParseSearchQuery(t *testing.T) {
	// Test that ParseSearchQuery returns a ParsedSearchQuery with the free text
	opts := ParseSearchQuery("hello world")
	assert.Equal(t, "hello world", opts.FreeText)

	// Test empty query
	opts = ParseSearchQuery("")
	assert.Equal(t, "", opts.FreeText)
}
