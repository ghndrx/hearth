...
func TestParseMentions(t *testing.T) {
	// parseMentions with non-UUID format returns empty slice
	mentions := parseMentions("Hello <@123456> and <@789012>!")

	// Non-UUID format mentions are not parsed - returns empty slice
	assert.Empty(t, mentions)

	// Test with valid UUID mentions
	validUUID := uuid.New()
	mentions2 := parseMentions("Hello <@" + validUUID.String() + ">!")
	assert.Len(t, mentions2, 1)
	assert.Equal(t, validUUID, mentions2[0])
}
...
func TestDeleteMessage_ServerChannelWithManagePermission(t *testing.T) {
	...
	ctx := context.Background()
	authorID := uuid.New()
	requesterID := uuid.New() // Different user with MANAGE_MESSAGES permission
	messageID := uuid.New()
	channelID := uuid.New()
	serverID := uuid.New()
	...
	defaultRole := &models.Role{
		ID:          serverID,
		Permissions: int64(0), // No additional permissions from @everyone
	}
	...
	// User has MANAGE_MESSAGES permission, should succeed
	assert.NoError(t, err)
}
