...
func (m *MockServerRepoForInvite) GetMembersPaginated(ctx context.Context, serverID uuid.UUID, cursor *models.MemberCursor, limit int) (*models.PaginatedMembers, error) {
...
func (m *MockServerRepoForInvite) GetMembersWithRole(ctx context.Context, serverID, roleID uuid.UUID) ([]*models.Member, error) {
...
func (m *MockServerRepoForInvite) GetInviteByVanityCode(ctx context.Context, vanityCode string) (*models.Invite, error) {
...
func (m *MockServerRepoForInvite) LogInviteUse(ctx context.Context, log *models.InviteUseLog) error {
	args := m.Called(ctx, log)
	return args.Error(0)
}

func (m *MockServerRepoForInvite) GetInviteUseLogs(ctx context.Context, inviteCode string) ([]models.InviteUseLog, error) {
...
func (m *MockServerRepoForInvite) GetServerInviteUseLogs(ctx context.Context, serverID uuid.UUID) ([]models.InviteUseLog, error) {
	args := m.Called(ctx, serverID)
	if args.Get(0) == nil {
		return nil, args.Error(1)
	}
...
func newTestInviteService() (*InviteService, *MockInviteRepository, *MockBanRepository, *MockServerRepoForInvite, *MockCacheService, *MockEventBus) {
	...
	// Pass nil for permService to bypass permission checks in tests
	// Tests that need permission checking should create service with a mock PermissionService
	service := NewInviteService(inviteRepo, banRepo, serverRepo, nil, cache, eventBus)
...
// newTestInviteServiceWithPermissions creates an InviteService with PermissionService for permission tests
func newTestInviteServiceWithPermissions() (*InviteService, *MockInviteRepository, *MockBanRepository, *MockServerRepoForInvite, *MockRoleRepository, *MockCacheService, *MockEventBus) {
	inviteRepo := new(MockInviteRepository)
	banRepo := new(MockBanRepository)
	serverRepo := new(MockServerRepoForInvite)
	...
	permService := NewPermissionService(serverRepo, roleRepo, nil, nil)
	service := NewInviteService(inviteRepo, banRepo, serverRepo, permService, cache, eventBus)

	return service, inviteRepo, banRepo, serverRepo, roleRepo, cache, eventBus
}

func TestInviteService_CreateInvite_PermissionDenied(t *testing.T) {
	service, _, _, serverRepo, roleRepo, _, _ := newTestInviteServiceWithPermissions()
	ctx := context.Background()
	...
	// Default role without CREATE_INVITE permission
...
func TestInviteService_BanMember_PermissionDenied(t *testing.T) {
	service, _, _, serverRepo, roleRepo, _, _ := newTestInviteServiceWithPermissions()
	ctx := context.Background()
	...
	server := &models.Server{
		ID:      serverID,
		OwnerID: ownerID, // Different from moderator
	}
	...
	defaultRole := &models.Role{
		...
		Name:        "@everyone",
		IsDefault:   true,
		Permissions: 0, // No BAN_MEMBERS permission
	}

	serverRepo.On("GetMember", ctx, serverID, userID).Return(targetMember, nil)
	serverRepo.On("GetMember", ctx, serverID, moderatorID).Return(moderatorMember, nil)
	serverRepo.On("GetByID", ctx, serverID).Return(server, nil)
	roleRepo.On("GetMemberPermissions", ctx, serverID, moderatorID).Return(int64(0), nil)
	roleRepo.On("GetDefaultRole", ctx, serverID).Return(defaultRole, nil)

	err := service.BanMember(ctx, serverID, userID, moderatorID, "")

	assert.Error(t, err)
	assert.Contains(t, err.Error(), "permission")
}

func TestInviteService_BanMember_RoleHierarchyViolation(t *testing.T) {
	service, _, _, serverRepo, roleRepo, _, _ := newTestInviteServiceWithPermissions()
	ctx := context.Background()
	...
	// Moderator has BAN_MEMBERS but target has higher role
	moderatorMember := &models.Member{
		ServerID: serverID,
		UserID:   moderatorID,
		JoinedAt: time.Now(),
	...
	// Target has higher-position admin role
	targetMember := &models.Member{
		ServerID: serverID,
		UserID:   userID,
		JoinedAt: time.Now(),
	...
	adminRole := &models.Role{
		...
		Name:        "Admin",
		Position:    100,
		Permissions: int64(models.PermAdministrator),
	}
	...
	memberRole := &models.Role{
		...
		Name:        "Moderator",
		Position:    50,
		Permissions: int64(models.PermBanMembers), // Has BAN permission but lower role
	}
	...
	serverRepo.On("GetMember", ctx, serverID, moderatorID).Return(moderatorMember, nil)
	serverRepo.On("GetByID", ctx, serverID).Return(server, nil)
	roleRepo.On("GetMemberPermissions", ctx, serverID, moderatorID).Return(int64(models.PermBanMembers), nil)
	roleRepo.On("GetDefaultRole", ctx, serverID).Return(defaultRole, nil)
	// GetMemberRoles is called to check role hierarchy
	roleRepo.On("GetMemberRoles", ctx, serverID, moderatorID).Return([]*models.Role{memberRole}, nil)
	roleRepo.On("GetMemberRoles", ctx, serverID, userID).Return([]*models.Role{adminRole}, nil)

	err := service.BanMember(ctx, serverID, userID, moderatorID, "")

	assert.Error(t, err)
	assert.Equal(t, ErrCannotManageMember, err)
}

func TestInviteService_GetServerBans_PermissionDenied(t *testing.T) {
	service, _, _, serverRepo, roleRepo, _, _ := newTestInviteServiceWithPermissions()
	ctx := context.Background()
	...
	bans, err := service.GetServerBans(ctx, serverID, requesterID)
...
func TestInviteService_GetServerInvites_PermissionDenied(t *testing.T) {
	service, _, _, serverRepo, roleRepo, _, _ := newTestInviteServiceWithPermissions()
	ctx := context.Background()
	...
	defaultRole := &models.Role{
		...
		Name:        "@everyone",
		IsDefault:   true,
		Permissions: 0, // No MANAGE_SERVER permission
	}
	...
	invites, err := service.GetServerInvites(ctx, serverID, requesterID)
