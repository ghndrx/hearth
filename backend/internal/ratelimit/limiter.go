...
var (
	// API rate limits - tuned for production load (per-IP)
	APIDefault = Config{Limit: 1000, Window: time.Minute}
	APIAuth    = Config{Limit: 300, Window: time.Second}
	APIUpload  = Config{Limit: 50, Window: time.Minute}

	// Message rate limits - tuned for throughput (per-user)
	MessageSend     = Config{Limit: 500, Window: time.Second}
	MessageEdit     = Config{Limit: 100, Window: time.Minute}
	MessageReaction = Config{Limit: 200, Window: time.Minute}
