...
func TestConfigValues(t *testing.T) {
	...
	assert.Equal(t, 1000, APIDefault.Limit)
	assert.Equal(t, time.Minute, APIDefault.Window)

	assert.Equal(t, 300, APIAuth.Limit)
	assert.Equal(t, time.Second, APIAuth.Window)

	assert.Equal(t, 500, MessageSend.Limit)
	assert.Equal(t, time.Second, MessageSend.Window)
