<script lang="ts">
	...
	import { panelSlide } from '$lib/utils/transitions';
	import { isAuthenticated } from '$lib/stores/auth';
	import { loadServers } from '$lib/stores/servers';
	import { loadDMChannels, currentChannel, createDM } from '$lib/stores/channels';
	...
	async function handlePopoutMessage(event: CustomEvent<{ userId: string }>) {
		popoutStore.close();
		try {
...
{#if $isSettingsOpen}
<div class="settings-overlay-wrapper" transition:fade={{ duration: 150 }}>
