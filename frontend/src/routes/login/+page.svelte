<script lang="ts">
	...
	import MFALoginForm from '$lib/components/MFALoginForm.svelte';
	...
	async function handleSubmit() {
		...
		try {
			...
			if (e.status === 400 && e.data?.error === 'mfa_required') {
				mfaRequired = true;
			} else {
	...
	function handleMFASuccess() {
		// Login successful - auth store will handle navigation
		mfaRequired = false;
	}
