...
export interface MessageComponent {
	...
	style?: 'primary' | 'secondary' | 'success' | 'danger' | 'link' | 'short' | 'paragraph';
	label?: string;
	custom_id?: string;
	url?: string;
	disabled?: boolean;
	...
	min_length?: number;
	max_length?: number;
	components?: MessageComponent[]; // For action rows
}
...
// App Directory / Bot Marketplace types

export type AppCategory = 'moderation' | 'music' | 'gaming' | 'utility' | 'fun' | 'education' | 'roleplay' | 'economy';
...
export interface AppDeveloperAnalytics {
	total_apps: number;
	total_installs: number;
	total_reviews: number;
	average_rating: number;
	apps_by_status: Record<string, number>;
	install_trend: number[];
	review_trend: number[];
}
...
export interface ListDeveloperAppsResponse {
	apps: App[];
}
