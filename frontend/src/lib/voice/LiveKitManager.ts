...
// Track event callbacks for video integration
export interface TrackEventCallbacks {
	onTrackSubscribed?: (track: Track, participantId: string, isScreenShare: boolean) => void;
	onTrackUnsubscribed?: (track: Track, participantId: string, isScreenShare: boolean) => void;
}
...
class LiveKitManager {
	...
	private trackEventCallbacks: TrackEventCallbacks = {};
	...
	private setupRoomListeners() {
		...
		this.room.on('connectionStateChanged' as any, (state: ConnectionState) => {
		...
		// Participant joined
		this.room.on('participantConnected' as any, (...args: any[]) => {
			const participant = args[0] as RemoteParticipant;
			console.log('[LiveKit] Participant joined:', participant.identity);
			this.updateParticipantsList();
		});

		// Participant left
		this.room.on('participantDisconnected' as any, (...args: any[]) => {
			const participant = args[0] as RemoteParticipant;
		...
		this.room.on('trackSubscribed' as any, (...args: any[]) => {
			const track = args[0] as Track;
			const participant = args[2] as RemoteParticipant;
			...
			if (track.kind === Track.Kind.Audio) {
				...
				// For video tracks, notify callback for VideoGrid integration
				// Don't auto-attach to DOM body - let VideoGrid handle it
				const isScreenShare = track.source === Track.Source.ScreenShare;
				if (this.trackEventCallbacks.onTrackSubscribed) {
					this.trackEventCallbacks.onTrackSubscribed(track, participant.identity, isScreenShare);
				}
			}
		});

		// Track unsubscribed
		this.room.on('trackUnsubscribed' as any, (...args: any[]) => {
			const track = args[0] as Track;
			const participant = args[2] as RemoteParticipant;
			
			if (track.kind === Track.Kind.Video) {
				const isScreenShare = track.source === Track.Source.ScreenShare;
				if (this.trackEventCallbacks.onTrackUnsubscribed) {
					this.trackEventCallbacks.onTrackUnsubscribed(track, participant.identity, isScreenShare);
				}
			}
		...
		this.room.on('activeSpeakersChanged' as any, (...args: any[]) => {
		...
		this.room.on('participantMetadataChanged' as any, () => {
			this.updateParticipantsList();
		});

		this.room.on('trackMuted' as any, () => {
			this.updateParticipantsList();
		});

		this.room.on('trackUnmuted' as any, () => {
			this.updateParticipantsList();
		});

		// Disconnected
		this.room.on('disconnected' as any, (...args: any[]) => {
			const reason = args[0] as DisconnectReason | undefined;
	...
	private cleanup() {
		...
		// Remove all audio and video elements
		document.querySelectorAll('[id^="voice-audio-"]').forEach(el => el.remove());
		document.querySelectorAll('[id^="voice-video-"]').forEach(el => el.remove());
	...
	/**
	 * Set track event callbacks for video integration
	 */
	setTrackEventCallbacks(callbacks: TrackEventCallbacks): void {
		this.trackEventCallbacks = callbacks;
	}
	...
	getRemoteParticipant(participantId: string): RemoteParticipant | undefined {
		...
		return this.room.remoteParticipants.get(participantId);
	}

	/**
	 * Get all remote participants with their video tracks
	 */
	getRemoteParticipantsWithTracks(): Array<{
		participant: RemoteParticipant;
		cameraTrack: RemoteTrackPublication | undefined;
		screenShareTrack: RemoteTrackPublication | undefined;
	}> {
		if (!this.room) return [];
		...
		this.room.remoteParticipants.forEach((participant) => {
			result.push({
				participant,
				cameraTrack: participant.getTrackPublication(Track.Source.Camera),
