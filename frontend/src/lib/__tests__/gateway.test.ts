...
describe('Gateway', () => {
	...
	describe('reconnection', () => {
		it('should attempt reconnection on close with non-1000 code', () => {
			...
			// Set readyState to CLOSED so reconnect's connect() doesn't early-return
	...
	describe('event handling', () => {
		...
		it('should handle TYPING_START events', () => {
			...
			gateway.connect('test-token');
			mockWs.onopen?.();
			logSpy.mockClear(); // Clear the "[Gateway] Connected" log
			...
			expect(logSpy).toHaveBeenCalledWith('[Gateway] Typing start:', typingData);
		});
		...
		it('should handle PRESENCE_UPDATE events', () => {
			...
			expect(logSpy).toHaveBeenCalledWith('[Gateway] Presence update:', presenceData);
		});
		...
		it('should handle unknown event types', () => {
			...
			expect(logSpy).toHaveBeenCalledWith('[Gateway] Unknown event:', 'UNKNOWN_EVENT', eventData);
		});
	});
