 ...
 */
export function generateIdempotencyKey(type: QueueActionType, _data: unknown): string {
  // Use UUID v4 as base, but include type for debugging
  // Note: _data reserved for future deterministic key generation
...
export async function markFailed(id: string, error: string): Promise<void> {
  ...
  const unsubscribe = queueStore.subscribe(queue => {
    currentItem = queue.find(i => i.id === id);
  });
  unsubscribe();
  
  if (currentItem) {
    const existingItem = currentItem as QueueItem;
    const newRetryCount = existingItem.retryCount + 1;
    const shouldFail = newRetryCount >= existingItem.maxRetries;
    
    const updatedItem = await updateQueueItem(id, {
      status: shouldFail ? 'failed' : 'pending',
      retryCount: newRetryCount,
      lastError: error
    });
    
    if (updatedItem && shouldFail) {
      emitQueueEvent(updatedItem, 'failed');
    }
  }
}
