...
type SyncHandler = (item: QueueItem<unknown>) => Promise<{ success: boolean; error?: string; data?: unknown }>;

const syncHandlers: Partial<Record<QueueActionType, SyncHandler>> = {
  SEND_MESSAGE: async (item: QueueItem<unknown>) => {
    const payload = item.data as SendMessagePayload;
  ...
  EDIT_MESSAGE: async (item: QueueItem<unknown>) => {
    const payload = item.data as EditMessagePayload;
  ...
  DELETE_MESSAGE: async (item: QueueItem<unknown>) => {
    const payload = item.data as DeleteMessagePayload;
  ...
  ADD_REACTION: async (item: QueueItem<unknown>) => {
    const payload = item.data as ReactionPayload;
  ...
  REMOVE_REACTION: async (item: QueueItem<unknown>) => {
    const payload = item.data as ReactionPayload;
  ...
  TYPING_INDICATOR: async (item: QueueItem<unknown>) => {
    // Typing indicators are ephemeral - if offline, just skip
    const payload = item.data as TypingPayload;
 ...
 */
function handleServiceWorkerSync(data: unknown): void {
