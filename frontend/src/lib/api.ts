...
// fetchApi - low-level fetch wrapper that returns the raw Response
// Used by stream.ts for streaming API calls that need raw response access
export async function fetchApi(path: string, options: RequestInit = {}): Promise<Response> {
	const headers: Record<string, string> = {
		...(options.headers as Record<string, string> || {})
	};
	
	// Get auth token from localStorage or module variable
	let token: string | null = null;
	if (browser) {
		token = localStorage.getItem('hearth_token');
		if (token && token !== authToken) {
