<script lang="ts">
	...
	function saveCustomStatus() {
		// Persist custom status to the dedicated API endpoint
		const statusPayload: Record<string, unknown> = {};
		if (customStatusText) statusPayload.custom_text = customStatusText;
		if (selectedEmoji) statusPayload.emoji = selectedEmoji;

		if (customStatusText || selectedEmoji) {
			api.put('/users/@me/status', statusPayload).catch((err) => {
				console.error('Failed to save custom status:', err);
			});
		} else {
			api.delete('/users/@me/status').catch((err) => {
				console.error('Failed to clear custom status:', err);
			});
		}

		gateway.updatePresence(currentStatus, currentStatus === 'idle',
			(customStatusText || selectedEmoji) ? { custom_text: customStatusText || undefined, emoji: selectedEmoji || undefined } : undefined
		);
