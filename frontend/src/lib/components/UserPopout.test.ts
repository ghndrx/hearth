...
describe('UserPopout', () => {
	...
	describe('member info', () => {
		...
		it('shows server member since date when member is provided', () => {
			...
			// Component shows "Member Since" when a server member is provided
			expect(getByText('Member Since')).toBeInTheDocument();
		});

		it('shows Hearth member since date when no member', () => {
			const { getByText } = render(UserPopout, {
				props: { user: mockUser }
			});
			// Component shows "Hearth Member Since" when no server member is provided
			expect(getByText('Hearth Member Since')).toBeInTheDocument();
		});
	...
	describe('mutual servers', () => {
		...
		it('shows +N more when more than 5 mutual servers', () => {
			const manyServers = Array.from({ length: 5 }, (_, i) => ({
				id: `server-${i}`,
				name: `Server ${i}`,
				icon: null
			}));
			const { getByText } = render(UserPopout, {
				props: { 
					user: mockUser, 
					mutualServers: manyServers,
					totalMutual: { servers: 8, channels: 0, friends: 0 }
				}
			});
	...
	describe('events', () => {
		...
		it('has message button for user interaction', async () => {
			...
			const messageBtn = container.querySelector('.action-btn-primary') as HTMLElement;
