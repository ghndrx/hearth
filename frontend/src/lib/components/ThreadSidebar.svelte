<script lang="ts">
	 ...
	 * 
	 * Enhanced thread sidebar view with:
	 * - Thread participant indicators
	 * - Thread notification controls (persisted to backend)
	 * - Active viewer presence indicators
	 * - Better thread context display
	 */
	...
	// FEAT-001: Get notification level from store (persisted to backend)
	$: threadNotificationSetting = $threadNotificationLevel;

	// FEAT-001: Get active viewers from store (real-time presence)
	$: activeViewers = $threadActiveViewers;

	// Extract unique participants from thread messages
	$: participants = getUniqueParticipants($threadMessages, $currentThread?.parent_message);

	// Cleanup on destroy
	onDestroy(() => {
		// Thread store handles cleanup when closed
	});

	function getUniqueParticipants(messages: any[], parentMessage: any) {
		...
		if (parentMessage?.author) {
			seen.add(parentMessage.author.id);
			result.push({
				id: parentMessage.author.id,
				username: parentMessage.author.username,
				display_name: parentMessage.author.display_name,
				avatar: parentMessage.author.avatar
			});
		}
	...
	async function setThreadNotification(level: ThreadNotificationLevel) {
		...
		try {
			// FEAT-001: Persist to backend via thread store
			await threadStore.setNotificationPreference(level);
...
{#if $currentThread}
	<aside class="thread-sidebar" aria-label="Thread: {threadTitle}" role="complementary">
		<!-- Thread Header -->
		<header class="thread-header">
			<div class="thread-header-content">
				...
				<div class="thread-header-actions">
					...
					<div class="notification-menu-container relative">
						<button 
							class="action-btn" 
							on:click={toggleNotificationMenu}
							title="Thread notification settings"
							aria-label="Thread notification settings"
							aria-expanded={showNotificationMenu}
							aria-haspopup="menu"
							type="button"
						>
							{#if threadNotificationSetting === 'none'}
						...
						{#if showNotificationMenu}
							<div class="notification-dropdown" role="menu">
								<div class="dropdown-header">Thread Notifications</div>
								<button 
									class="dropdown-item"
									class:active={threadNotificationSetting === 'all'}
									on:click={() => setThreadNotification('all')}
									role="menuitemradio"
									aria-checked={threadNotificationSetting === 'all'}
								>
									...
									<div class="dropdown-item-content">
										<span class="dropdown-item-title">All Messages</span>
										<span class="dropdown-item-desc">Notify for every reply</span>
									</div>
									{#if threadNotificationSetting === 'all'}
										<svg class="check-icon" width="16" height="16" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
								...
								<button 
									class="dropdown-item"
									class:active={threadNotificationSetting === 'mentions'}
									on:click={() => setThreadNotification('mentions')}
									role="menuitemradio"
									aria-checked={threadNotificationSetting === 'mentions'}
								>
									<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" aria-hidden="true">
									...
									<div class="dropdown-item-content">
										<span class="dropdown-item-title">Only @mentions</span>
										<span class="dropdown-item-desc">Notify only when mentioned</span>
									</div>
									{#if threadNotificationSetting === 'mentions'}
										<svg class="check-icon" width="16" height="16" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
											<path d="M9 16.17L4.83 12l-1.42 1.41L9 19 21 7l-1.41-1.41z"/>
										</svg>
									{/if}
								...
								<button 
									class="dropdown-item"
									class:active={threadNotificationSetting === 'none'}
									on:click={() => setThreadNotification('none')}
									role="menuitemradio"
									aria-checked={threadNotificationSetting === 'none'}
								>
									<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" aria-hidden="true">
										<path d="M13.73 21a2 2 0 0 1-3.46 0"></path>
										<path d="M18.63 13A17.89 17.89 0 0 1 18 8"></path>
									...
									<div class="dropdown-item-content">
										<span class="dropdown-item-title">Muted</span>
										<span class="dropdown-item-desc">No notifications from this thread</span>
									</div>
									{#if threadNotificationSetting === 'none'}
										<svg class="check-icon" width="16" height="16" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
											<path d="M9 16.17L4.83 12l-1.42 1.41L9 19 21 7l-1.41-1.41z"/>
					...
					<button class="action-btn close-btn" on:click={close} title="Close thread" aria-label="Close thread panel" type="button">
						<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" aria-hidden="true">
			...
			<!-- Thread participants bar -->
			{#if participants.length > 0}
				<div class="thread-participants-bar">
					<ThreadParticipants 
						{participants} 
						maxDisplay={5} 
						size="sm"
			...
			<!-- FEAT-001: Active viewers indicator (real-time presence) -->
			{#if activeViewers.length > 0}
				<div class="active-viewers-bar">
					<div class="active-indicator"></div>
					<span class="active-viewers-text">
						{#if activeViewers.length === 1}
							<strong>{activeViewers[0].display_name || activeViewers[0].username}</strong> is viewing
						{:else if activeViewers.length === 2}
							<strong>{activeViewers[0].display_name || activeViewers[0].username}</strong> and 
							<strong>{activeViewers[1].display_name || activeViewers[1].username}</strong> are viewing
						{:else}
							<strong>{activeViewers[0].display_name || activeViewers[0].username}</strong> and 
							{activeViewers.length - 1} others are viewing
						{/if}
					</span>
					<div class="active-viewers-avatars">
						{#each activeViewers.slice(0, 3) as viewer}
							<div class="viewer-avatar" title="{viewer.display_name || viewer.username}">
								<Avatar
									src={viewer.avatar}
						...
						{/each}
						{#if activeViewers.length > 3}
							<div class="viewer-overflow">+{activeViewers.length - 3}</div>
						{/if}
					</div>
				</div>
			{/if}
		...
		{#if parentMessage}
			<div class="parent-message">
				<div class="parent-message-header">
					<span class="parent-label">Thread started from</span>
				</div>
...
<style>
	...
	/* FEAT-001: Active viewers bar (real-time presence) */
	.active-viewers-bar {
		display: flex;
		align-items: center;
		gap: 8px;
	...
	.dropdown-item.active {
		background-color: var(--brand-primary-alpha, rgba(88, 101, 242, 0.15));
	}
