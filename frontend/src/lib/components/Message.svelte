...
<script lang="ts">
	...
	import StickerMessage from './stickers/StickerMessage.svelte';
	import MessageComponents from './message/MessageComponents.svelte';
	...
	<div class="flex-1 min-w-0 py-0.5 pr-4">
		...
		{:else if message.sticker_id}
			...
			<StickerMessage
				...
				stickerName={message.sticker_name || 'Sticker'}
				stickerUrl={message.sticker_url || message.content}
				stickerFormat={message.sticker_format || 'PNG'}
			/>
		{:else}
		...
		{#if message.sticker_id}
			<div class="mt-2 flex items-center gap-2" role="img" aria-label="Sticker">
				<div class="sticker-container">
					<img 
						src="/stickers/{message.sticker_id}.png" 
						alt="Sticker"
						class="sticker-image"
						title="Sticker"
					/>
...
.reaction-inactive:hover {
	transform: scale(1.1);
}
.reaction-active:active,
.reaction-inactive:active {
