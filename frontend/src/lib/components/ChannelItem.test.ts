...
describe('ChannelItem', () => {
	...
	describe('muted and unread states', () => {
		...
		it('shows mention badge when unread with mention count > 0', () => {
			const { container } = render(ChannelItem, {
				props: { channel: makeChannel(), unread: true, mentionCount: 5 }
			});
		...
		it('shows unread dot when unread with mention count = 0', () => {
			const { container } = render(ChannelItem, {
				props: { channel: makeChannel(), unread: true, mentionCount: 0 }
			});
		...
		it('does not show unread indicator when not unread', () => {
			const { container } = render(ChannelItem, {
				props: { channel: makeChannel(), unread: false, mentionCount: 0 }
			});
		...
		it('caps mention badge display at 99+ for large counts', () => {
			const { container } = render(ChannelItem, {
				props: { channel: makeChannel(), unread: true, mentionCount: 150 }
			});
