...
const mockCategorizedChannelsStore = writable<{ categories: any[]; uncategorized: any[] }>({ categories: [], uncategorized: [] });
const mockUserStore = writable<any>(null);
const mockAuthStore = writable<any>({ token: null, user: null });
...
vi.mock('$lib/stores/channels', () => {
	return {
		...
		categorizedChannels: mockCategorizedChannelsStore,
...
// Mock e2ee to prevent transitive auth dependency (gateway -> messages -> e2ee -> auth)
vi.mock('$lib/e2ee', () => ({
	isInitialized: () => false,
	decryptMessage: vi.fn(),
	deserializeEncryptedPayload: vi.fn()
