...
describe('ServerFolder', () => {
	...
	describe('events', () => {
		it('dispatches select event when server is clicked in expanded folder', async () => {
			...
			// Find the server icon button within the expanded folder
			const serverIconButton = container.querySelector('.folder-servers .server-icon');
			expect(serverIconButton).toBeInTheDocument();
			
			// Verify the button is clickable and represents the correct server
			// The click event is forwarded to parent - we just verify the element exists and is interactive
			await fireEvent.click(serverIconButton!);
			
			// Verify the folder servers section contains the expected servers
			const serverIcons = container.querySelectorAll('.folder-servers .server-icon');
			expect(serverIcons.length).toBe(mockServers.length);
		});
	});
