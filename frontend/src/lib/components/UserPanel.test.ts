...
const { mockUserStore, mockAuthStore, mockSettingsStore } = vi.hoisted(() => {
	return {
		mockUserStore: {
...
// Mock e2ee to break transitive dependency chain (messages -> e2ee -> auth)
vi.mock('$lib/e2ee', () => ({
	isInitialized: () => false,
...
describe('UserPanel', () => {
	...
	describe('status colors', () => {
		// Note: jsdom computes CSS vars to their fallback RGB values
		it('shows online status color by default', () => {
			const { container } = render(UserPanel, { props: { status: 'online' } });
			const statusDot = container.querySelector('.status-dot');
			const styles = getComputedStyle(statusDot!);
			expect(styles.backgroundColor).toMatch(/rgb\(35,\s*165,\s*89\)|#23a559/i);
		});
		...
		it('shows idle status color', () => {
			...
			const statusDot = container.querySelector('.status-dot');
			const styles = getComputedStyle(statusDot!);
			expect(styles.backgroundColor).toMatch(/rgb\(240,\s*178,\s*50\)|#f0b232/i);
		});
		...
		it('shows dnd status color', () => {
			...
			const statusDot = container.querySelector('.status-dot');
			const styles = getComputedStyle(statusDot!);
			expect(styles.backgroundColor).toMatch(/rgb\(242,\s*63,\s*67\)|#f23f43/i);
		});
		...
		it('shows offline status color', () => {
			...
			const statusDot = container.querySelector('.status-dot');
			const styles = getComputedStyle(statusDot!);
			expect(styles.backgroundColor).toMatch(/rgb\(128,\s*132,\s*142\)|#80848e/i);
		});
	});
