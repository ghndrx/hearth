...
{#if visible}
	...
	>
		...
		<main class="chat-main">
				...
				{#if $currentConversation}
					<div class="header-title">
						{#if editingTitle}
							...
							/>
						{:else}
							<button type="button" class="chat-title-btn" on:click={startEditTitle} on:keydown={(e) => e.key === 'Enter' && startEditTitle()} title="Click to rename">
			...
			{#if showSettings && $currentConversation}
				<div class="settings-panel" transition:slide>
					...
					<div class="setting-row">
						<label for="temperature-input">Temperature: {$currentConversation.temperature.toFixed(1)}</label>
						<input
		...
		<!-- Templates Modal -->
		{#if showTemplates}
			<div class="modal-overlay" on:click={() => (showTemplates = false)} on:keydown={(e) => e.key === 'Escape' && (showTemplates = false)} role="button" tabindex="0" transition:fade>
				<div class="modal-content" on:click|stopPropagation on:keydown role="dialog" aria-modal="true" tabindex="0" transition:fly={{ y: 20 }}>
					<div class="modal-header">
						<h2>Chat Templates</h2>
						<button class="icon-btn" on:click={() => (showTemplates = false)} aria-label="Close templates modal">
							<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
								<path d="M18 6L6 18M6 6l12 12" />
							</svg>
						</button>
