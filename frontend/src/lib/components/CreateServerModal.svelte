<script lang="ts">
	...
	async function handleCreate() {
		...
		try {
			...
			if (templateCode) {
				...
				const response = await api.post<{ id: string; name: string; icon_url?: string; owner_id: string; created_at: string }>(
					`/templates/${templateCode}/use`,
					{ name: name.trim() }
				);
				server = {
					id: response.id,
					name: response.name,
					icon: response.icon_url ?? null,
					banner: null,
					description: null,
				...
				server = await createServer(name.trim());
			}
	...
	function handleTemplateSelect(e: CustomEvent<{ code: string; name: string }>) {
		templateCode = e.detail.code;
		showTemplatePicker = false;
		// Auto-fill name with template name if empty
		if (!name) {
			name = e.detail.name;
		}
	}
...
<Modal {open} title={modalTitle} subtitle={modalSubtitle} size="small" on:close={close}>
		...
		<form on:submit|preventDefault={handleCreate} class="form">
			...
			{#if templateCode}
				<div class="template-indicator">
					...
					<button class="clear-template" on:click={clearTemplate} type="button">Remove</button>
				</div>
			{:else}
				<button class="browse-templates" on:click={() => (showTemplatePicker = true)} type="button">
	...
	<TemplatePickerModal
		...
		on:close={() => (showTemplatePicker = false)}
	/>
...
<style>
	...
	.browse-templates {
		...
		margin-bottom: 16px;
		background: var(--bg-secondary, #2b2d31);
		border: 1px solid var(--bg-modifier-accent, #404249);
		border-radius: 4px;
