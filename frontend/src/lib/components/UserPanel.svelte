<script lang="ts">
	...
	let userInfoBtn: HTMLButtonElement;
	let statusDotBtn: HTMLButtonElement;
	...
	// Current custom status from user data
	$: currentCustomStatus = $user?.custom_status || '';
	...
	function openUserPopout(event: MouseEvent) {
		if (!$user) return;
	...
	function openStatusPicker(event: MouseEvent) {
		event.stopPropagation();
		if (!statusDotBtn) return;
		const rect = statusDotBtn.getBoundingClientRect();
		statusPickerX = rect.left;
		statusPickerY = window.innerHeight - rect.top + 8;
		showStatusPicker = true;
	}

	function handleStatusChange(event: CustomEvent<{ status: PresenceStatus; customStatus?: string; emoji?: string }>) {
		const { status: newStatus } = event.detail;

		// Update presence store for real-time UI feedback
		presenceStore.setStatus(newStatus);

		// Custom status persistence is handled by UserStatusPicker directly via PUT /users/@me/status
...
<div class="user-panel" role="region" aria-label="User controls">
	<button class="user-info" bind:this={userInfoBtn} on:click={openUserPopout} aria-label="View profile for {$user?.display_name || $user?.username}" type="button">
		<div class="avatar">
			{#if $user?.avatar}
	...
	<!-- Status picker button positioned over the avatar's status dot -->
	<button
		class="status-dot-btn"
		bind:this={statusDotBtn}
		on:click={openStatusPicker}
	...
	{#if showStatusPicker}
		<UserStatusPicker
			show={true}
			anchorX={statusPickerX}
			anchorY={statusPickerY}
			currentStatus={status}
			currentCustomStatus={currentCustomStatus}
			on:close={() => showStatusPicker = false}
...
<style>
	...
	.status-dot-btn:hover .status-dot {
