<script lang="ts">
  ...
  // MFA state
  let mfaSetupModalOpen = false;
  let mfaDisabling = false;
  let mfaError = '';
  let disableMFAPassword = '';
  let showDisableMFAForm = false;
  ...
  function handleMFASetupComplete() {
    ...
    // Optionally refresh user data to show MFA as enabled
    // This would depend on how the user model tracks MFA status
  }
  ...
  async function handleDisableMFA() {
    ...
    try {
      await auth.disableMFA(disableMFAPassword);
      showDisableMFAForm = false;
      disableMFAPassword = '';
      // Optionally refresh user data
    } catch (err: any) {
      mfaError = err.message || 'Failed to disable MFA';
    } finally {
      mfaDisabling = false;
    }
  }
              ...
              <div class="mb-10 pb-10 border-b border-[var(--bg-modifier-accent)]">
                ...
                <div class="flex justify-between items-center mt-4 p-4 bg-[var(--bg-secondary)] rounded-lg">
                  <div>
                    ...
                    {#if mfaError}
                      <p class="text-[var(--status-danger)] text-sm mt-1">{mfaError}</p>
                    {/if}
                  </div>
                  <div class="flex gap-2">
                    <!-- For now, we'll assume MFA is not enabled. In a real implementation, this would check user.mfa_enabled -->
                    {#if false}
                      <!-- MFA is enabled -->
                      <span class="px-3 py-1 rounded-full bg-[var(--status-positive)] text-white text-xs font-medium">Enabled</span>
                      <button
                        class="px-4 py-2 rounded bg-[var(--status-danger)] text-white text-sm font-medium cursor-pointer transition-colors hover:bg-[var(--status-danger-hover)]"
                        on:click={handleShowDisableMFA}
                ...
                {#if showDisableMFAForm}
                  <div class="mt-4 p-4 bg-[var(--bg-modifier-accent)] rounded-lg border border-[var(--status-danger)]">
                    <h4 class="text-sm font-semibold text-[var(--text-primary)] mb-2">Disable Two-Factor Authentication</h4>
                    <p class="text-[var(--text-secondary)] text-xs mb-3">Enter your password to confirm:</p>
                    <div class="flex gap-2">
                      <input
                        type="password"
                        placeholder="Current password"
                        bind:value={disableMFAPassword}
                        class="flex-1 px-3 py-2 bg-[var(--input-bg)] border border-[var(--border)] rounded text-[var(--text-primary)] text-sm"
                        disabled={mfaDisabling}
                      />
                      <button
                        class="px-3 py-2 bg-[var(--status-danger)] text-white text-sm font-medium rounded hover:bg-[var(--status-danger-hover)] disabled:opacity-50"
                        on:click={handleDisableMFA}
                        disabled={mfaDisabling || !disableMFAPassword.trim()}
                      >
                        {mfaDisabling ? 'Disabling...' : 'Disable'}
                      </button>
                      <button
                        class="px-3 py-2 bg-[var(--bg-modifier-selected)] text-[var(--text-primary)] text-sm font-medium rounded hover:bg-[var(--bg-modifier-hover)]"
                        on:click={handleCancelDisableMFA}
                        disabled={mfaDisabling}
                      >
                        Cancel
                      </button>
            ...
            <section>
              ...
              <div class="mb-10 pb-10 border-b border-[var(--bg-modifier-accent)]">
                ...
                <div class="flex gap-4 flex-wrap">
                  <button
                    class="bg-transparent border-2 {appSettings.theme === 'system' ? 'border-[var(--brand-primary)]' : 'border-transparent'} rounded-lg p-2 cursor-pointer text-center hover:border-[var(--brand-primary)]"
                    on:click={() => setTheme('system')}
                  >
                    <div class="w-[120px] h-20 rounded overflow-hidden flex">
                      <div class="w-[60px] bg-[#1e1f22] flex flex-col justify-center px-1 gap-1">
                        <div class="h-3 bg-[#2b2d31] rounded-sm"></div>
                        <div class="h-3 bg-[#2b2d31] rounded-sm w-3/5"></div>
                      </div>
                      <div class="w-[60px] bg-[#f2f3f5] flex flex-col justify-center px-1 gap-1">
                        <div class="h-3 bg-[#e3e5e8] rounded-sm"></div>
                        <div class="h-3 bg-[#e3e5e8] rounded-sm w-3/5"></div>
                      </div>
                    </div>
                    <span class="block mt-2 text-[var(--text-primary)] text-sm">System</span>
                  </button>
...
<!-- MFA Setup Modal -->
<MFASetupModal
  bind:open={mfaSetupModalOpen}
  on:close={() => mfaSetupModalOpen = false}
  on:complete={handleMFASetupComplete}
/>
