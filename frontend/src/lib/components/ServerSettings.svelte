<script lang="ts">
	...
	import StickerManager from './stickers/StickerManager.svelte';
	import TemplateCreator from './TemplateCreator.svelte';
	import EventManager from './events/EventManager.svelte';
	import WebhookManager from './WebhookManager.svelte';
	import ForumTagEditor from './ForumTagEditor.svelte';
	...
	let newChannel: { name: string; type: 'text' | 'voice' | 'announcement' | 'forum'; topic: string } = {
		name: '',
		type: 'text',
		topic: ''
	};
	...
	$: forumChannels = $serverChannels.filter(c => c.type === 6);
				...
				<div class="content-scroll">
					...
					{:else if activeSection === 'templates'}
						<section class="templates-section">
							...
							<p class="section-desc">
								Create a template from this server to share its structure with others, 
								or use a template when creating a new server.
							</p>
							<TemplateCreator 
								serverId={$currentServer.id} 
								{isOwner}
						...
						<section>
							...
							<div class="settings-section">
								...
								{#if forumChannels.length === 0}
									<p class="no-items">No forum channels</p>
								{:else}
									{#each forumChannels as channel}
										<div class="forum-channel-section">
											<div class="channel-item">
												<span class="channel-icon">&#128172;</span>
												<span class="channel-name">{channel.name}</span>
												{#if isOwner}
											...
											<div class="forum-tags-section">
												<ForumTagEditor channelId={channel.id} canManage={isOwner} />
											</div>
										</div>
									{/each}
								{/if}
					...
					{:else if activeSection === 'webhooks'}
						<section>
							<h1>Webhooks</h1>
							<p class="section-description">Webhooks allow external services to send messages into your server channels.</p>
							{#if $currentServer}
								<WebhookManager
									serverId={$currentServer.id}
		...
		<Modal open={true} on:close={() => showCreateChannelModal = false}>
			<div class="create-channel-modal">
				...
				<div class="channel-type-select">
					...
					<label class="channel-type-option" class:selected={newChannel.type === 'forum'}>
						<input type="radio" value="forum" bind:group={newChannel.type} />
						<span class="type-icon">💬</span>
						<div>
							<span class="type-name">Forum</span>
							<span class="type-desc">Create tagged discussion posts</span>
						</div>
					</label>
				</div>
...
<style>
	...
	/* Forum Channel Sections */
	.forum-channel-section {
		margin-bottom: 16px;
	...
	.forum-channel-section .channel-item {
		border-radius: 0;
