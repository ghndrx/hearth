...
function createAuthStore() {
	...
	return {
		...
		async loginWithMFA(email: string, password: string, mfaCode: string) {
			update(s => ({ ...s, loading: true }));
			authDebug('Starting MFA login', { email });

			try {
				const response = await api.post<{ access_token: string; refresh_token: string }>('/auth/login/mfa', {
					email,
					password,
					mfa_code: mfaCode
				});

				authDebug('MFA login response received', {
					hasAccessToken: !!response?.access_token,
					hasRefreshToken: !!response?.refresh_token
				});
				...
				authDebug('MFA login complete', { userId: user.id });
				...
				authDebug('MFA login failed', { error });
				update(s => ({ ...s, loading: false }));
				throw error;
			}
		},

		async enableMFA(): Promise<{ secret: string; qr_code_url: string; backup_codes: string[] }> {
			const response = await api.post<{ secret: string; qr_code_url: string; backup_codes: string[] }>('/auth/mfa/enable');
			return response;
		},

		async verifyMFASetup(code: string): Promise<void> {
			await api.post<{ message: string }>('/auth/mfa/verify', { code });
		},

		async disableMFA(password: string): Promise<void> {
			await api.post<{ message: string }>('/auth/mfa/disable', { password });
		},
