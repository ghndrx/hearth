...
import { handleMessageCreate, handleMessageUpdate, handleMessageDelete, handleReactionAdd, handleReactionRemove, handleReactionRemoveAll } from './messages';
...
function createGatewayStore() {
  ...
  async function handleDispatch(type: string, data: unknown, _sequence?: number) {
    ...
    switch (type) {
      ...
      case 'MESSAGE_CREATE': {
        ...
        await handleMessageCreate(normalized);
        break;
      }
      ...
      case 'REACTION_ADD': {
        const reactionData = data as { message_id: string; channel_id: string; user_id: string; emoji: string };
        handleReactionAdd(reactionData);
        break;
      }

      case 'REACTION_REMOVE': {
        const reactionData = data as { message_id: string; channel_id: string; user_id: string; emoji: string };
        handleReactionRemove(reactionData);
        break;
      }

      case 'REACTION_REMOVE_ALL': {
        const reactionData = data as { message_id: string; channel_id: string };
        handleReactionRemoveAll(reactionData);
        break;
      }
      ...
      case 'USER_STATUS_UPDATE':
        console.log('[Gateway] User status update:', data);
        break;
  ...
  function updatePresence(status: string, afk = false, customStatus?: { custom_text?: string; emoji?: string }) {
    send({
      op: Op.PRESENCE_UPDATE,
      d: { status, afk, custom_status: customStatus },
    });
  }
