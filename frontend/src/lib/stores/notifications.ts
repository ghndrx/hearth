...
export interface Notification {
	id: string;
	user_id: string;
	type: 'mention' | 'reply' | 'direct_message' | 'friend_request' | 'friend_accept' | 'server_invite' | 'server_join' | 'reaction' | 'system';
	title: string;
	body: string;
	read: boolean;
...
function createNotificationStore() {
	...
	async function loadStats() {
		try {
			const stats = await api.get<NotificationStats>('/users/@me/notifications/stats');
			update(s => ({ ...s, stats }));
		} catch {
